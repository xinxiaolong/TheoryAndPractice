...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		Bitmap bitmap = null;
		try {
			File imageFile = configuration.diskCache.get(uri); //根据Uri获取对应的图片文件

			//文件不为空 说明已经缓存到了本地硬盘 可以尝试从本地硬盘取
			...
			//没能尝试从硬盘取成功(文件不存在，文件被损坏),就去从网络下载
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                //先判断是否要缓存到本地硬盘?再去下载和存储到本地硬盘:直接执行下一步
				if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {
					imageFile = configuration.diskCache.get(uri);
					//如果保存成功，将imageUriForDecoding指向本地文件路径
					if (imageFile != null) {
				...
				//根据imageUriForDecoding去获取相应地bitmap  imageUriForDecoding可能分为两种
				//1:缓存到了本地的imageUriForDecoding=file类型
				//2:没有缓存到本地的imageUriForDecoding=原始的uri 会从网上下载本图片
	...
	/**
	 * 根据解码图片的目标路径 结合 imageLoadingInfo 得到相应地 Bitmap
	 * @param imageUri
	 * @return
	 * @throws IOException
	 */
	...
	private void checkTaskNotActual() throws TaskCancelledException {
		checkViewCollected();//判断用于显示图片的View是否被GC回收
		checkViewReused();  //判断显示图片的View跟绑定的Uri是否一致  engine.prepareDisplayTaskFor(imageAware, memoryCacheKey);
	}
